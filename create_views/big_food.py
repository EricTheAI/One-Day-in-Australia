...
'''
function(doc) {
constraint_1 = (doc.metadata.json.place.name == 'Melbourne' || doc.metadata.json.place.name == 'Sydney')
...
emit([doc.metadata.json.place.name,'junkfood',doc._id,doc.metadata.json.coordinates.coordinates,doc.metadata.json.created_at], 1); 
...
constraint_2 = doc.metadata.json.text.match(regex)
if(constraint_1 && constraint_2) 
emit([doc.metadata.json.place.name,'healthyfood',doc._id,doc.metadata.json.coordinates.coordinates,doc.metadata.json.created_at], 1); 
}''',
