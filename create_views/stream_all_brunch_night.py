...
if(constraint_1) 
emit([doc.metadata.place.name,'all',doc._id,doc.metadata.coordinates.coordinates,doc.metadata.created_at], 1); 
}''')
view.sync(db)
...
if(constraint_1 && constraint_2 && constraint_3)  
emit([doc.metadata.place.name,'nightout',doc._id,doc.metadata.coordinates.coordinates,doc.metadata.created_at], 1); 
}''')
view.sync(db)
